...
SELECT p.apellido1, p.apellido2, p.nombre, d.nombre AS departamento FROM profesor pr JOIN persona p ON p.id = pr.id_profesor JOIN departamento d ON d.id = pr.id_departamento ORDER BY p.apellido1 ASC, p.apellido2 ASC, p.nombre ASC;
...
SELECT a.nombre AS nombre, ce.anyo_inicio AS anyo_inicio, ce.anyo_fin AS anyo_fin FROM alumno_se_matricula_asignatura ama JOIN persona p ON p.id = ama.id_alumno JOIN asignatura a ON a.id = ama.id_asignatura JOIN curso_escolar ce ON ce.id = ama.id_curso_escolar WHERE p.nif = '26902806M';
...
SELECT DISTINCT d.nombre AS nombre FROM departamento d JOIN profesor pr ON pr.id_departamento = d.id JOIN asignatura a ON a.id_profesor = pr.id_profesor JOIN grado g ON g.id = a.id_grado WHERE g.nombre = 'Grado en Ingeniería Informática (Plan 2015)';
...
SELECT pe.apellido1, pe.apellido2, pe.nombre FROM persona AS pe INNER JOIN profesor AS pr ON pr.id_profesor = pe.id LEFT JOIN asignatura AS a ON a.id_profesor = pr.id_profesor WHERE a.id_profesor IS NULL ORDER BY pe.apellido1, pe.apellido2, pe.nombre;

-- 14. Retorna un llistat amb les assignatures que no tenen un professor/a assignat. (id, nombre)
SELECT a.id, a.nombre FROM asignatura AS a LEFT JOIN profesor AS p ON a.id_profesor = p.id_profesor WHERE p.id_profesor IS NULL ORDER BY a.nombre;

-- 15. Retorna un llistat amb tots els departaments que no han impartit assignatures en cap curs escolar. (nombre)
SELECT d.nombre FROM asignatura AS a RIGHT JOIN profesor AS p ON a.id_profesor = p.id_profesor RIGHT JOIN departamento AS d ON p.id_departamento = d.id WHERE a.id IS NULL ORDER BY d.nombre;
...
SELECT d.nombre AS departamento, COUNT(p.id_profesor) AS total FROM departamento AS d INNER JOIN profesor AS p ON p.id_departamento = d.id GROUP BY d.nombre ORDER BY num_professors DESC;
...
SELECT g.nombre AS grau, COUNT(a.id) AS total FROM grado AS g LEFT JOIN asignatura AS a ON a.id_grado = g.id GROUP BY g.nombre ORDER BY num_assignatures DESC;
...
SELECT g.nombre AS grau, COUNT(a.id) AS total FROM grado AS g LEFT JOIN asignatura AS a ON a.id_grado = g.id GROUP BY g.nombre HAVING COUNT(a.id) > 40 ORDER BY num_assignatures DESC;
...
SELECT g.nombre AS grau, a.tipo AS tipus, SUM(a.creditos) AS total_creditos FROM grado AS g INNER JOIN asignatura AS a ON a.id_grado = g.id GROUP BY g.nombre, a.tipo ORDER BY g.nombre, a.tipo;
