...
SELECT pe.apellido1, pe.apellido2, pe.nombre FROM persona AS pe LEFT JOIN asignatura a ON pe.id = a.id_profesor WHERE a.id IS NULL AND pe.tipo = 'profesor';
...
SELECT DISTINCT d.nombre FROM departamento AS d LEFT JOIN profesor AS p ON d.id = p.id_profesor LEFT JOIN asignatura AS a ON p.id_profesor = a.id_profesor WHERE a.id IS NULL;
